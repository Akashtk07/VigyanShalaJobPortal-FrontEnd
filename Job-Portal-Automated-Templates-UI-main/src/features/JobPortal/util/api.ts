...
const apiClient = (options: AxiosRequestConfig = {}) => {
...
export const getActivePostedJobs = async (pageNumber: number) => {
  return await apiClient().get(
    `/job/student/job/active?pageNumber=${pageNumber}`
  );
};
...
export const searchJobs = async (data: any, pageNumber: number) => {
  return await apiClient().get(
    `/job/admin/jobs/?pageNumber=${pageNumber}`,
    data
  );
};
